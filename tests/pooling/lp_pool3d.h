...
nt::Tensor backward_lp_pool3d(nt::Tensor input, nt::Tensor output_grad, 
                                nt::Scalar power, nt::utils::my_n_tuple<3> kernel_size, nt::utils::my_n_tuple<3> stride = -1,
    ...
    if(ceil_mode){
        ...
        if((dout-1) * stride[0] >= (din+padding[0]))
            ...
            modR = kernel_size[0] - ((din+(2*padding[0]) - kernel_size[0]) % stride[0]);
    ...
    if (ceil_mode){input = input.pad({0, modD, 0, modR, 0, modC}); in_shape = input.shape().clone();}
    Scalar one(complex_64(1,1));
    ...
    strided_a = strided_a.unfold(-3, kernel_size[1], stride[1]).unfold(-3, kernel_size[2], stride[2]);
...
nt::Tensor adaptive_lp_pool3d(nt::Tensor x, nt::utils::my_n_tuple<3> out_shape, nt::Scalar power){
    using namespace nt;
    ...
    return lp_pool3d(x, power, {kernel_size_d, kernel_size_r, kernel_size_c}, {stride_d, stride_r, stride_c});
}
