...
void plotPersistentDiagram(
    const std::vector<std::vector<std::tuple<Tensor, double, double>>>
        &_homologyData) {
    //make sure deaths and births are processed once for the graphs
    std::vector<std::vector<std::tuple<Tensor, double, double>>> homologyData;
    homologyData.reserve(_homologyData.size());
    for(const auto& _data : _homologyData){
        homologyData.push_back(_data);
        std::vector<std::tuple<Tensor, double, double>>& data = homologyData.back();
        std::sort(data.begin(), data.end(), [](const auto& a, const auto& b) -> bool 
                  {return std::get<2>(a) < std::get<2>(b);});
        auto last = std::unique(data.begin(), data.end(), [](const auto& a, const auto& b){
                                    return std::get<2>(a) == std::get<2>(b) && std::get<1>(a) == std::get<1>(b);
                            });
