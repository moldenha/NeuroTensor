...
void make_trig_test_torch(std::string name, bool in_functional, std::string args = "", std::string bounds="(-1, 1)"){
...
#define NT_MAKE_RUN_TEST(name, in_functional, _rtol, _atol)\
    run_test(#name " autograd test", []{\
        ...
        make_trig_test_torch(n, in_functional);\
        ...
        nt::utils::throw_exception(\
            ...
            "Error, grads do not match $ \n$ \n$ \n$ \n$ \n$ \n$ \n$", \
             ...
             nt::isclose(x1.grad(), expected_x1grad, _rtol, _atol, true), nt::isclose(x2.grad(), expected_x2grad, _rtol, _atol, true));\
        nt::utils::throw_exception(\
            ...
            "Error, grads do not match $ \n$ \n$ \n$ \n$ \n$ \n$", \
             ...
             nt::isclose(w1.grad(), expected_w1grad, _rtol, _atol, true), nt::isclose(w2.grad(), expected_w2grad, _rtol, _atol, true)); \
        nt::utils::throw_exception(\
            ...
            "Error, grads do not match $ \n$ \n$ \n$ \n$ \n$ \n$", \
             ...
             nt::isclose(b1.grad(), expected_b1grad, _rtol, _atol, true), nt::isclose(b2.grad(), expected_b2grad, _rtol, _atol, true));\
    });\

void trig_test_autograd(){
    ...
    NT_MAKE_RUN_TEST(sin, false, 1e-3, 1e-4);
    NT_MAKE_RUN_TEST(asin, false, 1e-5, 1e-8);
    NT_MAKE_RUN_TEST(asinh, false, 1e-5, 1e-8);

    NT_MAKE_RUN_TEST(cosh, false, 1e-3, 1e-4);
    NT_MAKE_RUN_TEST(cos, false, 1e-5, 1e-8);
    NT_MAKE_RUN_TEST(acos, false, 1e-3, 1e-4);
    NT_MAKE_RUN_TEST(acosh, false, 1e-5, 1e-8);

}

#undef ADD_UNDERSCORE
#undef NT_MAKE_RUN_TEST
