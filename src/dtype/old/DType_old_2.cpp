...
d_type::d_type(const uint8_t _l):type(DType::uint8),data(_l){}
d_type::d_type(const int8_t _l):type(DType::int8),data(_l){}
d_type::d_type(const uint16_t _l):type(DType::uint16),data(_l){}
d_type::d_type(const int16_t _l):type(DType::int16),data(_l){}
d_type::d_type(const std::complex<float> _l):type(DType::cfloat),data(_l){}
d_type::d_type(const std::complex<double> _l):type(DType::cdouble),data(_l){}
d_type::d_type(const int64_t _l):type(DType::int64),data(_l){}
d_type::d_type(const uint_bool_t _l):type(DType::Bool),data(_l){}
...
T d_type::cast_num() const{
	switch(type){
		case DType::Integer:
			return (T)std::get<int32_t>(data);
		case DType::Long:
			return (T)std::get<uint32_t>(data);
		case DType::Double:
			return (T)std::get<double>(data);
		case DType::Float:
			return (T)std::get<float>(data);
		case DType::uint8:
			return (T)std::get<uint8_t>(data);
		case DType::int8:
			return (T)std::get<int8_t>(data);
		case DType::int16:
			return (T)std::get<int16_t>(data);
		case DType::uint16:
			return (T)std::get<uint16_t>(data);
		case DType::LongLong:
			return (T)std::get<int64_t>(data);
		default:
			return 0;
	}
}
...
template int d_type::cast_num<int32_t>() const;
template double d_type::cast_num<double>() const;
template float d_type::cast_num<float>() const;
template int64_t d_type::cast_num<int64_t>() const;
template uint16_t d_type::cast_num<uint16_t>() const;
template int16_t d_type::cast_num<int16_t>() const;
template uint8_t d_type::cast_num<uint8_t>() const;
template int8_t d_type::cast_num<int8_t>() const;
...
d_type_reference& d_type_reference::operator=(const int32_t& inp){
	switch(type){
		...
		case DTypeConst::LongLong:
			...
			break;
		case DTypeConst::Bool:
			std::get<11>(data).get() = uint_bool_t(inp > 0 ? true : false);
			break;
		case DTypeConst::TensorObj:
...
template<typename T>
T d_type_reference::operator+(const T& inp) const{
	return item<T>() + inp;
}
...
std::ostream& operator << (std::ostream& out, const d_type_reference &ref){
	switch(ref.type){
		case DTypeConst::Float:
			out << std::get<0>(ref.data).get();
			break;
		case DTypeConst::Double:
			out << std::get<1>(ref.data).get();
			break;
		case DTypeConst::Complex64:
			out << std::get<2>(ref.data).get().real()<<","<<std::get<2>(ref.data).get().imag();
			break;
		case DTypeConst::Complex128:
			out << std::get<3>(ref.data).get().real()<<","<<std::get<3>(ref.data).get().imag();
			break;
		case DTypeConst::uint8:
			out << std::get<4>(ref.data).get();
			break;
		case DTypeConst::int8:
			out << std::get<5>(ref.data).get();
			break;
		case DTypeConst::int16:
			out << std::get<6>(ref.data).get();
			break;
		case DTypeConst::uint16:
			out << std::get<7>(ref.data).get();
			break;
		case DTypeConst::Integer:
			out << std::get<8>(ref.data).get();
			break;
		case DTypeConst::Long:
			out << std::get<9>(ref.data).get();
			break;
		case DTypeConst::LongLong:
			out << std::get<10>(ref.data).get();
			break;
		case DTypeConst::Bool:
			out << std::get<11>(ref.data).get();
			break;
		case DTypeConst::TensorObj:
			out << std::get<12>(ref.data).get();
			break;
		default:
			break;
	}
