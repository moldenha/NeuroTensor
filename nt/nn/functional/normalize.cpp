...
TensorGrad TensorGrad_Functional_Class::instance_norm(const TensorGrad& x, utils::optional_tensor running_mean, 
                                                      utils::optional_tensor running_var,
                                                    utils::optional_tensorgrad weight, utils::optional_tensorgrad bias,
                                                    bool use_input_stats, Scalar momentum, Scalar eps){
    if(!x.track_grad()){
        if(!weight.has_value() && !bias.has_value()){
            Tensor out = ::nt::functional::no_grad::instance_norm(
                x.detach(), running_mean, running_var,
                nullptr, nullptr, use_input_stats,
                momentum, eps);
            TensorGrad result(std::move(out), x.track_grad());
            result.track_grad_(false);
            return std::move(result);
        }
        return TensorGrad_Functional_Class::instance_norm(x.detach(), 
                                                          running_mean, running_var, 
                                                          weight, bias, 
                                                          use_input_stats, momentum, eps);
    }
    utils::throw_exception(x.dims() >= 3, "Error, cannot do instance norm on tensor with dims less than 3, got $",
                           x.shape());
    bool get_weight_grad = false;
    bool get_bias_grad = false;
    if(weight.has_value() && weight.value().track_grad())
    ...
    result.create_backward_function(
        ...
        (const Tensor &grad, std::vector<intrusive_ptr<TensorGrad>> &parents,
         intrusive_ptr<tensor_holder> original_weight, intrusive_ptr<tensor_holder> original_input,
         intrusive_ptr<tensor_holder> stored_means, intrusive_ptr<tensor_holder> stored_inv) {
            Tensor out_grad = ::nt::functional::no_grad::instance_norm_backward(
                grad, original_input->tensor, 
                stored_means->tensor, stored_inv->tensor,
                original_weight->tensor,
                (!get_weight_grad && get_bias_grad) ? 
...
TensorGrad TensorGrad_Functional_Class::instance_norm(const Tensor& x, utils::optional_tensor running_mean, 
                                                      utils::optional_tensor running_var,
                                                    utils::optional_tensorgrad weight, utils::optional_tensorgrad bias,
                                                    bool use_input_stats, Scalar momentum, Scalar eps){
    if(!weight.has_value() && !bias.has_value()){
    ...
    if(bias.has_value() && bias.value().track_grad())
        get_bias_grad = true;
    if(get_weight_grad == false && get_bias_grad == false){
        Tensor out = ::nt::functional::no_grad::instance_norm(
            x, running_mean, running_var,
            utils::optional_tensor(weight->detach()), utils::optional_tensor(bias->detach()),
            use_input_stats, momentum, eps);
        TensorGrad result(std::move(out), false);
        result.track_grad_(false);
        return std::move(result);
