...
inline static constexpr auto im2col_nn_layer_nd = [](auto a_begin, auto a_end, auto b_begin, const uint32_t& k_r, const uint32_t& k_c, const uint32_t& s_r, const uint32_t& s_c, const uint32_t& d_r, const uint32_t& d_c, const uint32_t& t_r, const uint32_t& t_c, const uint32_t& LKern, const uint32_t& L, const uint32_t& channels, const uint32_t& batches){
	...
	uint32_t batch_add = 0;
	for(uint32_t ba = 0; ba < batches; ++ba, batch_add += (channels * mat_size)){
...
inline static constexpr auto im2col_nn_layer_nd_none = [](auto a_begin, auto a_end, auto b_begin, const uint32_t& k_r, const uint32_t& k_c, const uint32_t& t_r, const uint32_t& t_c, const uint32_t& LKern, const uint32_t& L, const uint32_t& channels, const uint32_t& batches){
	...
	const uint32_t kc_add = k_c-1; 
	const uint32_t batch_add = ((channels - 1) * mat_size) + ((k_r - 1) * t_c);
	const uint32_t a_currentCA = mat_size - ((k_r*k_c) + (a_current_add * k_r));
	for(uint32_t ba = 0; ba < batches; ++ba, a_begin += batch_add){
		for(uint32_t r = 0; r < upper_rows; ++r, a_begin += kc_add){
			for(uint32_t c = 0; c < upper_cols; ++c, ++a_begin){
				auto a_current = a_begin;
				for(uint32_t ch = 0; ch < channels; ++ch, a_current += a_currentCA){
...
//this would be a good optimization to add ans figure out for dilation when it isn't 2 am and you have more time to actually work on this
...
/* 	const uint32_t upper_rows2 = k_r*d_r; */
/* 	const uint32_t upper_cols2 = k_c * d_c; */
/* 	const uint32_t kc_add = (k_c-1) * d_c; */ 
/* 	/1* const uint32_t batch_add = ((channels - 1) * mat_size) + (((k_r-1) * d_r) * t_c); *1/ */
/* 	//it would be (d_c * k_c * k_r) */
/* 	const uint32_t a_currentCA = mat_size - ((d_c * k_c * k_r) + (a_current_add * (k_r-1))); */
/* 	std::cout << "a_currentCA: "<< a_currentCA << "," << ((d_c * k_c * k_r) + (a_current_add * k_r)) << "," << mat_size << "," << (d_c * k_c * k_r) << "," << a_current_add * k_r <<std::endl; */
/* 	auto a_begin_copy = a_begin; */
/* 	for(uint32_t ba = 0; ba < batches; ++ba){ */
/* 		uint32_t batch_add = (ba * channels * mat_size); */
/* 		a_begin = a_begin_copy + batch_add; */
...
inline static constexpr auto im2col_nn_layer_nd_stride = [](auto a_begin, auto a_end, auto b_begin, const uint32_t& k_r, const uint32_t& k_c, const uint32_t& s_r, const uint32_t& s_c, const uint32_t& t_r, const uint32_t& t_c, const uint32_t& LKern, const uint32_t& L, const uint32_t& channels, const uint32_t& batches){
	...
	const uint32_t upper_rows = t_r - (k_r - 1);
	const uint32_t upper_cols = t_c - (k_c - 1);
	const uint32_t kc_add = k_c-1 + (t_c * (s_r-1));
	const uint32_t a_currentCA = mat_size - ((k_r*k_c) + (a_current_add * k_r));
	const uint32_t batch_add = ((channels-1)*mat_size) + ((k_r - 1) * t_c); 
	for(uint32_t ba = 0; ba < batches; ++ba, a_begin += batch_add){
		for(uint32_t r = 0; r < upper_rows; r += s_r, a_begin += kc_add){
			for(uint32_t c = 0; c < upper_cols; c += s_c, a_begin += s_c){
				auto a_current = a_begin;
				for(uint32_t ch = 0; ch < channels; ++ch, a_current += a_currentCA){
					for(uint32_t rr = 0; rr < k_r; ++rr, a_current += a_current_add){
						for(uint32_t cc = 0; cc < k_c; ++cc, ++a_current, ++b_begin){
...
Tensor Unfold::forward(const Tensor& x){
	...
	int32_t L_r = ((x.shape()[-2] + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1) / stride[0]) + 1;
	int32_t L_c = ((x.shape()[-1] + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1) / stride[1]) + 1; 
	uint32_t L = L_r * L_c;
	BLSHAPE = L;
	BROWS = x.shape()[-2];
	BCOLS = x.shape()[-1];
...
Tensor Unfold::backward(const Tensor& dz) const{
	utils::throw_exception(dz.dims() == 3 || dz.dims() == 4, "Expected dimensions of Tensor to unfold backward to be 3 or 4 but got $", dz.dims());
	if(out_transpose){
		Tensor Z = (dz.dims() == 4) ? dz.flatten(0, -3) : dz;
		/* SizeRef outp_shape_ut({x.shape()[0], X.shape()[1]*LKern, L}); */
		uint32_t batches = dz.shape()[0];
		uint32_t channels = Z.shape()[1] / LKern;
		utils::throw_exception(Z.shape()[2] == BLSHAPE, "Expected backward of Unfold layer to have valid last index to have shape $ but got $", BLSHAPE, Z.shape()[2]);
		SizeRef output_shape({batches, channels, BROWS, BCOLS});
		Tensor X = functional::zeros(output_shape, dz.dtype);
		if(!(padding == 0)){
			X.arr_void().execute_function_nbool(im2col_nn_layer_nd_T_pad_backward, Z.arr_void(), kernel_size[0], kernel_size[1], stride[0], stride[1], dilation[0], dilation[1], padding[0], padding[1], BROWS, BCOLS, Z.shape()[1], BLSHAPE, channels, batches);
			return std::move(X);
		}
		X.arr_void().execute_function_nbool(im2col_nn_layer_nd_T_backward,  Z.arr_void(), kernel_size[0], kernel_size[1], stride[0], stride[1], dilation[0], dilation[1], BROWS, BCOLS, Z.shape()[1], BLSHAPE, channels, batches);
		return std::move(X);
	}
	Tensor Z = (dz.dims() == 4) ? dz.view({dz.shape()[0], dz.shape()[1] * dz.shape()[2], dz.shape()[3]}) : dz;
	uint32_t batches = dz.shape()[0];
	uint32_t channels = Z.shape()[2] / LKern;
	utils::throw_exception(Z.shape()[1] == BLSHAPE, "Expected backward of Unfold layer to have valid (1) index to have shape $ but got $", BLSHAPE, Z.shape()[1]);
	SizeRef output_shape({batches, channels, BROWS, BCOLS});
	Tensor X = functional::zeros(output_shape, dz.dtype);
	if(!(padding == 0)){
		X.arr_void().execute_function_nbool(im2col_nn_layer_nd_pad_backward, Z.arr_void(), kernel_size[0], kernel_size[1], stride[0], stride[1], dilation[0], dilation[1], padding[0], padding[1], BROWS, BCOLS, LKern, BLSHAPE, channels, batches);
		return std::move(X);
	}
	X.arr_void().execute_function_nbool(im2col_nn_layer_nd_backward,  Z.arr_void(), kernel_size[0], kernel_size[1], stride[0], stride[1], dilation[0], dilation[1], BROWS, BCOLS, LKern, BLSHAPE, channels, batches);	
