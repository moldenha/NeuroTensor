...
template<typename T, typename T2>
inline void run_inner_unfoldnd_backward_loop(
    size_t i,
    int64_t col_add,
    int64_t img_add,
    ...
    if (i == indexes.size()) {
        im_ptr[img_add] += col_ptr[col_add];
        return;
    }
    ...
    switch(remainder){
        case 0: do{
            NT_INNER_UNFOLDND_BACKWARD_TEXT__;
        case 7:
            NT_INNER_UNFOLDND_BACKWARD_TEXT__;
        case 6:
            NT_INNER_UNFOLDND_BACKWARD_TEXT__;
...
inline static constexpr auto im2col_cpu_nd_backward = [](auto data_im_ptr, auto data_im_ptr_end, auto data_col,
    const int64_t& channels,
        ...
        [&](threading::blocked_range<1> block) {
            for (int64_t batch = block.begin[0]; batch < block.end[0]; ++batch) {
                ...
                for (int64_t c = 0; c < channels_col; ++c) {
                    ...
                    std::vector<int64_t> indexes(dims);
                    run_inner_unfoldnd_backward_loop(0, col_add, im_add, indexes,
...
void unfoldnd_backward_(ArrayVoid &output, ArrayVoid &grad, const int64_t& channels,
    ...
    output.execute_function<WRAP_DTYPES<NumberTypesL> >(
       im2col_cpu_nd_backward, grad, channels, size, kernels, strides, paddings, dilations,
                    batch_size);
}


#undef NT_INNER_UNFOLDND_BACKWARD_TEXT__
