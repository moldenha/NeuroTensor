...
inline TensorGrad list(T&& first, Args&&... rest){
    ...
    result.parents.reserve(sizeof...(Args) + 1);
    result.parents.push_back(make_intrusive<TensorGrad>(first));
    ((result.parents.push_back(make_intrusive<TensorGrad>(rest))), ...);
    ...
    // register the result as a child for autograd tracking
    intrusive_ptr<TensorGrad> res_intrusive =
        make_intrusive<TensorGrad>(result);
    first.children->push_back(res_intrusive);
    ((rest.children->push_back(res_intrusive)), ...);
