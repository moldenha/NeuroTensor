...
nt::Tensor lp_pool2d(nt::Tensor input, nt::Scalar power, nt::utils::my_tuple kernel_size, nt::utils::my_tuple stride = -1, bool ceil_mode = false){
    using namespace nt;
...
nt::Tensor backward_lp_pool2d(nt::Tensor input, nt::Tensor output_grad, 
                                nt::Scalar power, nt::utils::my_tuple kernel_size, nt::utils::my_tuple stride = -1,
    ...
    if (ceil_mode){input = input.pad({0, modR, 0, modC}); in_shape = input.shape().clone();}
...
nt::Tensor adaptive_lp_pool2d(nt::Tensor x, nt::utils::my_tuple out_shape, nt::Scalar power){
    using namespace nt;
    ...
    return lp_pool2d(x, power, {kernel_size_r, kernel_size_c}, {stride_r, stride_c});
}
