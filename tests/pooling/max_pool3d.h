...
nt::Tensor max_pool3d(nt::Tensor input, 
                      nt::utils::my_n_tuple<3> kernel_size, nt::utils::my_n_tuple<3> stride = -1, nt::utils::my_n_tuple<3> padding = 0,
                      nt::utils::my_n_tuple<3> dilation=1, bool ceil_mode = false, bool return_indices = false, bool get_bools = false){
    using namespace nt;
    utils::throw_exception(dilation >= 1, "Expected dilation to be greater than or equal to one got $", dilation);
    ...
    if(!(padding == 0) || ceil_mode) input = input.pad({padding[0], padding[0]+modD, padding[1], padding[1]+modR, padding[2], padding[2]+modC}, "constant", -nt::inf);

    if(dilation > 1){
        if(!return_indices){
            return max_pool3d(input.undilate_(dilation[0], dilation[1], dilation[2]), kernel_size, stride, 0, 1, false, false);
        }
        auto [output, indices_] = get<2>(max_pool3d(input.undilate_(dilation[0], dilation[1], dilation[3]), kernel_size, stride, 0, 1, false, true, true));
        auto indices = indices_.dilate(dilation[0], dilation[1], dilation[2]);
        auto outShape = indices.shape().delete_index(-1);
        if(!(padding == 0) || ceil_mode) indices = unpad(indices, {padding[0], padding[0]+modD, padding[1], padding[1]+modR, padding[2], padding[2]+modC});
        Tensor out_indices = functional::where(indices.flatten(-3, -1))[-1].item<Tensor>();
    ...
    if(return_indices){
        ...
        strided_bools = strided_bools.unfold(-3, kernel_size[1], stride[1]).unfold(-3, kernel_size[2], stride[2]).flatten(-3, -1);
        ...
        // std::cout
        if(!(padding == 0) || ceil_mode) bools = unpad(bools, {padding[0], padding[0]+modD, padding[1], padding[1]+modR, padding[2], padding[2]+modC});
        Tensor out = functional::where(bools.flatten(-3, -1))[-1].item<Tensor>();
        return functional::list(out_values, out.view(out_shape));
    }
    return strided.max(-1).values;
}

nt::Tensor backward_max_pool3d(nt::SizeRef input_shape, nt::Tensor dldg, nt::Tensor indices){
    nt::Tensor grad = nt::functional::zeros(input_shape, dldg.dtype());
    nt::Tensor setting = nt::functional::at_tensor_split(grad.flatten(-1, -3), indices.flatten(-1,-3), -2);
    setting.set_(dldg.flatten(-1, -3));
    return std::move(grad);
}

nt::Tensor max_unpool3d(nt::Tensor input, nt::Tensor indices, nt::utils::my_n_tuple<3> kernel_size, nt::utils::my_n_tuple<3> stride, nt::utils::my_n_tuple<3> padding, nt::utils::my_n_tuple<3> output_size = -1){
    using namespace nt;
    utils::throw_exception(indices.dtype() == DType::int64,
                           "Max unpool requires indices to be int64 got $", indices.dtype());
    if(stride == -1) stride = kernel_size;
    int64_t c_out = (output_size[1] != -1 ? output_size[-1] : (input.shape()[-1] - 1) * stride[2] - 2 * padding[2] + kernel_size[2]);
    int64_t r_out = (output_size[1] != -1 ? output_size[-2] : (input.shape()[-2] - 1) * stride[1] - 2 * padding[1] + kernel_size[1]);
    int64_t d_out = (output_size[0] != -1 ? output_size[-3] : (input.shape()[-3] - 1) * stride[0] - 2 * padding[0] + kernel_size[0]);
    ...
    utils::throw_exception(d_out > 0,
                           "Error, output size $ cannot be less than or equal to 0", d_out);
    // if(!(padding == 0)) indices = indices + ((padding[0] * kernel_size[1] * kernel_size[2]) + (kernel_size[2] * padding[1]) + padding[2]);
    Tensor unpooled = functional::zeros(input.shape().redo_index(-1, c_out).redo_index(-2, r_out).redo_index(-3, d_out), input.dtype());
    Tensor setting = functional::at_tensor_split(unpooled.flatten(-3, -1), indices.flatten(-3, -1), -2);
    setting.set_(input.view(setting.shape()));
    return std::move(unpooled);
}

nt::Tensor backward_max_unpool3d(nt::SizeRef input_shape, nt::Tensor dldg, nt::Tensor indices, nt::utils::my_n_tuple<3> padding){
    using namespace nt;
    // if(!(padding == 0)){
    //     indices = indices + ((padding[0] * dldg.shape()[-2] * dldg.shape()[-1]) + (dldg.shape()[-2] * padding[1]) + padding[2]); 
    // }
    Tensor grad = functional::zeros(input_shape, dldg.dtype());
    Tensor f_grad = grad.flatten(-1,-3);
    functional::at_tensor_split(dldg.flatten(-1, -3), indices.flatten(-1,-3), -2, f_grad);
    return std::move(grad);
}

nt::Tensor adaptive_max_pool3d(nt::Tensor x, nt::utils::my_n_tuple<3> out_shape, bool return_indices = false){
    using namespace nt;
    ...
    return max_pool3d(x, {kernel_size_d, kernel_size_r, kernel_size_c}, {stride_d, stride_r, stride_c}, {padding_d, padding_r, padding_c}, 1, false, return_indices);
}
