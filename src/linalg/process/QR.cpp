...
std::tuple<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>, 
    ...
    QR_eigen(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& mat){
    ...
    if constexpr (std::is_floating_point_v<T>  || detail::is_eigen_complex<T>){
        ...
        utils::throw_exception(false, "Types not complex or floating are not supported for QR Decomposition");
        MatType Q(0,0);
        MatType R(0,0);
        return std::make_tuple(Q, R);
    }
...
#define _NT_QR_EIGEN_DECLARE_(type) template std::tuple<Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>,\
                                    Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>\
                                        QR_eigen<type>(Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>&);
