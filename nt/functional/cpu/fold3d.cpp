...
inline static constexpr auto col2im_cpu_3d = [](auto data_im_ptr,
    ...
    const int64_t col_matrix_size =
        batch_size * channels_col * depth_col * height_col * width_col;

    const int64_t batch_im_add = channels * depth * height * width;
    const int64_t batch_col_add =
        channels_col * depth_col * height_col * width_col;
            ...
            [&](threading::blocked_range<1> block) {
                for (int64_t batch = block.begin[0]; batch < block.end[0];
                    ...
                    for (int64_t c = 0; c < channels_col; ++c) {
                        ...
                        int64_t d_pad_start = std::max(
                            int64_t(0), (p_d - d_offset * d_d + s_d - 1) / s_d);
                        int64_t d_pad_end = std::min(
                            depth_col,
                            (depth + p_d - d_offset * d_d + s_d - 1) / s_d);
                        for (int64_t d = d_pad_start; d < d_pad_end; ++d) {
                            ...
                            for (int64_t h = h_pad_start; h < h_pad_end; ++h) {
                                ...
                                for (int64_t w = w_pad_start; w < w_pad_end;
                                     ++w) {
                                    int64_t w_pad =
                                        w * s_c - p_c + w_offset * d_c;
                                    data_im[c_im * depth * height * width +
                                            d_pad * height * width +
                                            h_pad * width + w_pad] +=
                                        data_col_ptr[c * height_col *
                                                         width_col * depth_col +
                                                     d * height_col *
                                                         width_col +
                                                     h * width_col + w];
        ...
        [&](auto range, auto data_im) { // works whether parallel or not, but
            ...
            for (int64_t batch = range.begin[0]; batch < range.end[0];
                ...
                for (int64_t c = 0; c < channels_col; ++c) {
                    ...
                    // Calculate the valid range for d_pad
                    int64_t d_pad_start =
                        std::max(int64_t(0), (p_d - d_offset * d_d + s_d - 1) / s_d);
                    int64_t d_pad_end = std::min(
                        depth_col,
                        (depth + p_d - d_offset * d_d + s_d - 1) / s_d);
                        ...
                        for (int64_t h = h_pad_start; h < h_pad_end; ++h) {
                            ...
                            for (int64_t w = w_pad_start; w < w_pad_end; ++w) {
                                ...
                                data_im[c_im * height * width * depth +
                                        d_pad * height * width + h_pad * width +
                                        w_pad] += *data_col_ptr;
                                ++data_col_ptr;
                            }
                    ...
                    // Adjust data_col_ptr for the skipped rows, depth, and
                    // columns
...
void fold3d_(ArrayVoid &im, ArrayVoid &col,
    ...
    im.execute_function < WRAP_DTYPES<NumberTypesL> >(
                                  col2im_cpu_3d, col, channels, depth, height,
                                  width, k_d, k_r, k_c, s_d, s_r, s_c, p_d, p_r,
                                  p_c, d_d, d_r, d_c, batch_size);
}
