...
void _convert(const ArrayVoid& in, ArrayVoid& out){
    in.cexecute_function<WRAP_DTYPES<NumberTypesL, DTypeEnum<DType::Bool> > >(
    [&out](auto in_begin, auto in_end){
        using from_t = utils::IteratorBaseType_t<decltype(in_begin)>;
        out.execute_function<WRAP_DTYPES<NumberTypesL, DTypeEnum<DType::Bool> > >(
        [&](auto o_begin, auto o_end){
            using to_t = utils::IteratorBaseType_t<decltype(o_begin)>;
            for(;o_begin != o_end; ++o_begin, ++in_begin){
                *o_begin = ::nt::convert::convert<to_t, from_t>(*in_begin);
            }
        });
    });
}
...
void _internal_convert_(ArrayVoid& in){
    in.execute_function<WRAP_DTYPES<NumberTypesL>>(
    [](auto begin, auto end){
        using from_t = utils::IteratorBaseType_t<decltype(begin)>;
        if constexpr (sizeof(from_t) == sizeof(T)){
            for(;begin != end; ++begin){
                *((T*)&(begin[0])) = ::nt::convert::convert<T, from_t>(*begin);
            }
        }
    });
}
...
void _internal_convert_(ArrayVoid& in, DType dt){
    ArrayVoid arr(1, dt);
    arr.execute_function<WRAP_DTYPES<NumberTypesL>>(
