...
BucketCPU::BucketCPU(nt::intrusive_ptr<DeviceHolder> buckets, nt::intrusive_tracked_list<void*> strides, int64_t strideS, int64_t bS, bool blocked, DType dt)
	:Bucket(dt),
    buckets_(buckets),
	strides_(strides),
	stride_size(strideS),
...
int64_t BucketCPU::blocked_stride_size() const{
	int64_t size = 0;
	/* std::cout << "doing blocked_stride_size"<<std::endl; */
	std::size_t dtype_s = DTypeFuncs::size_of_dtype(this->dtype_);
...
BucketCPU BucketCPU::blocked_strides_clone_cpu() const noexcept {
	BucketCPU output(size(), this->dtype_, device_type());
...
BucketCPU BucketCPU::blocked_strides_clone_tensor() const noexcept {
	int64_t nsize = size();
	nt::intrusive_ptr<DeviceHolder> nData = make_intrusive<DeviceHolder>(1);
	(*nData)[0] = make_device_(dCPU);
	...
	for(uint64_t i = 0; i < stride_size; ++i){
		...
		for(;sBegin != sEnd; ++sBegin, ++begin){
			*begin = sBegin->clone_cpu();
		}
	}
	return BucketCPU(std::move(nData), std::move(nStrides), 2, 1, true, this->dtype_);
}


BucketCPU BucketCPU::strided_clone_cpu() const noexcept{
	BucketCPU output(size(), this->dtype_, device_type());
	const std::size_t dtype_s = DTypeFuncs::size_of_dtype(this->dtype_);
	uint8_t* begin = reinterpret_cast<uint8_t*>((*output.buckets_)[0]->get_memory());
	for(uint64_t i = 0; i < stride_size; ++i){
		const uint8_t* sBegin = reinterpret_cast<const uint8_t*>(strides_[i]);
...
BucketCPU BucketCPU::strided_clone_tensor() const noexcept{
	int64_t nsize = size();
	nt::intrusive_ptr<DeviceHolder> nData = make_intrusive<DeviceHolder>(1);
	(*nData)[0] = make_device_(dCPU);
	(*nData)[0]->allocate_memory(DType::TensorObj, nsize);
	...
	for(;s_begin != s_end; ++s_begin, ++begin){
		*begin = (*s_begin)->clone_cpu();
	}
	return BucketCPU(std::move(nData), std::move(nStrides), 2, 1, true, this->dtype_);
}

uint64_t BucketCPU::getBucketCPUSize(const uint64_t bucket_index) const{
	if(!strides_blocked){return 1;}
	const uint32_t dtype_s = DTypeFuncs::size_of_dtype(this->dtype_);
...
uint64_t BucketCPU::bucket_index(uint64_t& index) const {
	const std::size_t dtype_s = DTypeFuncs::size_of_dtype(this->dtype_);
...
BucketCPU::BucketCPU(const int64_t size, DType dt, MemoryLayout mem_t)
	:Bucket(dt),
...
{
	(*buckets_)[0] = make_device_(DeviceType::CPU, mem_t);
...
BucketCPU::BucketCPU(const int64_t size, DType dt, void* ptr, DeleterFnPtr func)
	:Bucket(dt),
    buckets_(make_intrusive<DeviceHolder>(1)),
	strides_(2),
	bs(1),
	...
	strides_blocked(true)
{
	...
	(*buckets_)[0] = make_device_(dCPU, MemoryLayout::Private);
	...
	strides_blocked(true)
{
	...
	/* utils::THROW_EXCEPTION(1 == 0, "BucketCPU() default, data_nullptr: $ size_nullptr: $ bucket_sizes: $", data == nullptr, sizes == nullptr, bucket_sizes); */
	(*buckets_)[0] = make_device_(dCPU, MemoryLayout::Private); //by default on the cpu
	(*buckets_)[0]->allocate_memory(this->dtype_, 1);
	strides_[0] = (*buckets_)[0]->get_memory();
...
BucketCPU::BucketCPU(const BucketCPU& other)
	:Bucket(other.dtype_),
...
BucketCPU::BucketCPU(BucketCPU&& other)
	:Bucket(other.dtype_),
...
BucketCPU::BucketCPU(std::nullptr_t)
	:Bucket(DType::Float32)
    buckets_(nullptr),
	strides_(nullptr),
	stride_size(0),
...
bool BucketCPU::is_contiguous() const {
	if(strides_blocked && stride_size == 2){return true;}
	if(strides_blocked){return false;}
	const std::size_t type_size = DTypeFuncs::size_of_dtype(this->dtype_);
...
BucketCPU& BucketCPU::operator=(const BucketCPU& b){
	buckets_ = b.buckets_;
	strides_ = b.strides_;
...
BucketCPU& BucketCPU::operator=(BucketCPU&& b){
...
int64_t BucketCPU::size() const noexcept {
	if(strides_blocked){return blocked_stride_size();}
	return stride_size;
}

int64_t BucketCPU::storage_size() const noexcept {
    return DTypeFuncs::size_of_dtype(this->dtype_) * size();
}

BucketCPU BucketCPU::clone_cpu() const noexcept {
	if(strides_blocked){return (this->dtype_ == DType::TensorObj) ? blocked_strides_clone_tensor() : blocked_strides_clone_cpu();}
	return strided_clone_cpu();
}

BucketCPU BucketCPU::contiguous_cpu() const{
	if(this->dtype_ == DType::TensorObj){
		if(!is_contiguous()){return clone_cpu();}
		uint32_t type = iterator_type();
		if(type == 1){
			auto begin = cbegin_contiguous<Tensor>();
			auto end = cend_contiguous<Tensor>();
			for(;begin != end; ++begin){
				if(!begin->is_contiguous()){return clone_cpu();}
			}
			return *this;
		}
		else if(type == 2){
...
intrusive_ptr<Bucket> BucketCPU::force_contiguity_and_bucket() const{
	...
	if(i_type == 2){
		...
		if(bs == 1){
			...
			for(;begin < end; begin += d_size, ++o_begin)
				*o_begin = begin;
			return make_intrusive<BucketCPU>(buckets_, std::move(nStrides), n_size, bs, false, this->dtype_);
		}
		//this is for when there is a contiguous block of memory for every block
		if(bs == (stride_size/2)){
			return bucket_all_indices_cpu();
		}
	...
	if(bs == 1){
		...
		for(;begin != end; begin += d_size, ++o_begin)
			*o_begin = begin;	
		return make_intrusive<Bucket>(buckets_, std::move(nStrides), n_size, bs, false, this->dtype_);
	}
...
//and then buckets it
intrusive_ptr<Bucket> BucketCPU::bound_force_contiguity_bucket() const{
...
bool BucketCPU::can_force_contiguity() const{
	return bs == 1;
}
int64_t BucketCPU::force_contig_size() const {
	return reinterpret_cast<const uint8_t*>(data_ptr_end()) - reinterpret_cast<const uint8_t*>(data_ptr());
}

bool BucketCPU::can_force_contiguity_bytes(const int64_t& bytes) const{
...
intrusive_ptr<Bucket> BucketCPU::force_contiguity(int64_t n_size) const{
	n_size *= DTypeFuncs::size_of_dtype(this->dtype_);
	...
	return make_intrusive<BucketCPU>(buckets_, std::move(nStrides), 2, bs, true, this->dtype_);
}

BucketCPU BucketCPU::to_shared_cpu() const{
	if(this->is_shared())
		return *this;
	BucketCPU output(size(), this->dtype_, MemoryLayout::Shared);
	...
	if(type == 1){
		...
		void** s_begin = stride_begin();
		void** s_end = stride_end();
		const std::size_t d_size = DTypeFuncs::size_of_dtype(this->dtype_);
		for(;s_begin != s_end; ++s_begin){
			const uint8_t* sBegin = reinterpret_cast<const uint8_t*>(*s_begin);
...
BucketCPU BucketCPU::to_private_cpu() const{
	if(memory_layout() == MemoryLayout::Private)
		return *this;
	BucketCPU output(size(), this->dtype_, MemoryLayout::Private);
	uint8_t* o_begin = reinterpret_cast<uint8_t*>(output.data_ptr());
	uint32_t type = iterator_type();
	if(type == 1){
		const uint8_t* begin = reinterpret_cast<const uint8_t*>(data_ptr());
...
// BucketCPU BucketCPU::to_device(DeviceType dt) const {
// 	switch(dt){
// 		case DeviceType::META:
// 			utils::throw_exception(dt != DeviceType::META, "Cannot convert to meta");
// 		case DeviceType::CPU:
// 			return to_private_cpu();
// 		case DeviceType::CPUShared:
// 			return to_shared_cpu();
// 		default:
// 			return *this;
// 	}
// }

intrusive_ptr<Bucket> BucketCPU::to_mtl(MemoryLayout mem_t) const {
#ifndef NT_MTL_SUPPORTED
    utils::throw_exception(false, "Error, cannot convert cpu to an mtl device, apple metal performace shaders not supported on this device");
#else
    return intrusive_ptr<BucketMTL>(BucketMTL::from_cpu(*this, mem_t))
#endif
}

BucketCPU BucketCPU::new_bounds_cpu(uint64_t start, uint64_t end) const{
    ...
    if(!strides_blocked){
		...
		return BucketCPU(buckets_, strides_ + start, end-start, bs, false, this->dtype_);
	}
	if(is_contiguous()){
		...
		return BucketCPU(buckets_, std::move(nStrides), 2, 1, true, this->dtype_);
	}
	...
	for(;checked_end < stride_size; checked_end += 2){
		ntSize += getBucketCPUSize(checked_end / 2);
		if(end <= ntSize){break;}
	}
    ...
    if(ntSize == end){
        ...
        return BucketCPU(buckets_, std::move(nStrides), checked_end-checked_begin,
                                num_buckets, true, this->dtype_);
    }
    //the only other possibility is ntSize < end
    //TODO: copy the above, but the last stride will be the difference between (ntSize - getBucketCPUSize(checked_end - 2 / 2)) and end
    //similar to how start was used in the first stride
	uint64_t nstride_size = checked_end - checked_begin;
	nt::utils::THROW_EXCEPTION(nstride_size > 0, "nBS was 0 for new_bounds_cpu");
	...
	*arr = reinterpret_cast<uint8_t*>(*my_strides_begin) + (end);
	return BucketCPU(buckets_, std::move(nStrides), nstride_size, bs, true, this->dtype_);
}


BucketCPU BucketCPU::makeNullBucketCPU(DType dt, int64_t stride_size){
	intrusive_ptr<DeviceHolder> nData;
	intrusive_tracked_list<void*> nStrides(stride_size);
	return BucketCPU(std::move(nData), std::move(nStrides), stride_size, 0, false, dt);
}

BucketCPU BucketCPU::makeCopyBucketCPU(DType dt, const intrusive_ptr<DeviceHolder>& bucks, bool blocked, int64_t bS, int64_t stride_size){
	return BucketCPU(bucks, intrusive_tracked_list<void*>(stride_size), stride_size, bS, blocked, dt);
}

#ifndef USE_PARALLEL
template<>
BucketCPU BucketCPU::range_contiguous_<BucketCPU>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    ...
    BucketCPU out(this->buckets_, intrusive_tracked_list<void*>(out_stride_size * 2), out_stride_size * 2,
               this->bs, true, this->dtype_);
    int64_t counter = 0;
    for(const auto& pair : ranges){
...
BucketCPU BucketCPU::range_contiguous_<BucketCPU>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
	...
	const int64_t dtype_s = static_cast<int64_t>(DTypeFuncs::size_of_dtype(this->dtype_));
...
template<>
Tensor BucketCPU::range_contiguous_<Tensor>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    ArrayVoid out(this->range_contiguous_<BucketCPU>(std::move(ranges)));
...
#ifndef USE_PARALLEL
template<>
BucketCPU BucketCPU::range_strided_<BucketCPU>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    int64_t msize = this->size();
	...
	const int64_t dtype_s = static_cast<int64_t>(DTypeFuncs::size_of_dtype(this->dtype_));
    BucketCPU out(this->buckets_, intrusive_tracked_list<void*>(out_stride_size), out_stride_size,
               this->bs, false, this->dtype_);
    int64_t counter = 0;
    for(const auto& pair : ranges){
...
template<>
Tensor BucketCPU::range_strided_<Tensor>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    ArrayVoid out(this->range_strided_<BucketCPU>(std::move(ranges)));
    SizeRef ref({out.Size()});
    return Tensor(std::move(out), std::move(ref));
}
...
template<>
BucketCPU BucketCPU::range_bucketed_<BucketCPU>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    int64_t msize = this->size();
    for(const auto& pair : ranges){
        utils::THROW_EXCEPTION(pair.first < pair.second && pair.second <= msize,
                               "Pair {$, $} is not valid for a numel of $", 
...
template<>
Tensor BucketCPU::range_bucketed_<Tensor>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
    ArrayVoid out(this->range_bucketed_<BucketCPU>(std::move(ranges)));
    SizeRef ref({out.Size()});
    return Tensor(std::move(out), std::move(ref));
}

template <>
BucketCPU BucketCPU::range<BucketCPU>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
	if(!strides_blocked){return range_strided_<BucketCPU>(std::move(ranges));}
	if(is_contiguous()){return range_contiguous_<BucketCPU>(std::move(ranges));}
	return range_bucketed_<BucketCPU>(std::move(ranges));
}

template <>
Tensor BucketCPU::range<Tensor>(std::vector<std::pair<int64_t, int64_t>> ranges) const {
	if(!strides_blocked){
...
template <>
std::vector<BucketCPU> BucketCPU::split_contiguous_<std::vector<BucketCPU>>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
		return std::vector<BucketCPU > {*this};
	}
	uint64_t div = msize / splitting;
	uint64_t remainder = msize % splitting;
	bool r = remainder > 0;
	std::vector<BucketCPU > fb( (r) ? div + 1 : div, makeCopyBucketCPU(/*dt = */ dtype, /*buckets = */ this->buckets_, /*blocked = */ true, /*bucket size=*/ bs, 2));
...
#ifndef USE_PARALLEL
template <>
Tensor BucketCPU::split_contiguous_<Tensor>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
	...
	for(uint64_t i = 0; i < div; ++i, ++begin){
		...
		begin->_vals.bucket.dtype_ = this->dtype_;
...
template <>
std::vector<BucketCPU> BucketCPU::split_strided_<std::vector<BucketCPU>>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
		return std::vector<BucketCPU > {*this};
	}
	...
	std::vector<BucketCPU > fb( (r) ? div + 1 : div, BucketCPU::makeNullBucketCPU(this->dtype_));
	uint64_t current = 0;
	/* const int64_t adding = dtype_s * splitting; */
...
template <>
Tensor BucketCPU::split_strided_<Tensor>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
		Tensor output = Tensor::makeNullTensorArray(1);
		Tensor& t = output.item<Tensor>();
...
template <>
std::vector<BucketCPU> BucketCPU::split_bucketed_<std::vector<BucketCPU>>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
		return std::vector<BucketCPU > {*this};
	}
	...
	std::vector<BucketCPU > fb( (r) ? div + 1 : div, makeCopyBucketCPU(this->dtype_, buckets_, true, bs, 0)); //the 0 ensures that the fb[i].strides_ are holding nullptr's so no memory is allocated
	splitting *= dtype_s;
	remainder *= dtype_s;
...
template <>
Tensor BucketCPU::split_bucketed_<Tensor>(uint64_t splitting) const{
	int64_t msize = size();
	if(splitting > msize){
		Tensor output = Tensor::makeNullTensorArray(1);
		Tensor& t = output.item<Tensor>();
	...
	for(;begin_t != end_t; ++begin_t){
		...
		begin_t->_vals.bucket.dtype_ = this->dtype_;
...
template <>
std::vector<BucketCPU> BucketCPU::split<std::vector<BucketCPU>>(uint64_t splitting) const {
	if(!strides_blocked){return split_strided_<std::vector<BucketCPU>>(splitting);}
	if(is_contiguous()){return split_contiguous_<std::vector<BucketCPU>>(splitting);}
	return split_bucketed_<std::vector<BucketCPU>>(splitting);
}

template <>
Tensor BucketCPU::split<Tensor>(uint64_t splitting) const {
...
void BucketCPU::swap(BucketCPU& other){
	...
	std::swap(this->dtype_, other.dtype_);
	
}


const bool BucketCPU::is_sub_memory() const{
...
BucketCPU BucketCPU::bucket_all_indices_cpu() const{
	...
	return BucketCPU(buckets_, std::move(nStrides), n_stride_size, bs, false, this->dtype_);
}

intrusive_ptr<Bucket> BucketCPU::copy_strides(bool copy_vals) const {
	if(strides_blocked){return bucket_all_indices_cpu();}
	const int64_t n_stride_size = size();
	nt::intrusive_tracked_list<void*> nStrides(n_stride_size);
    if(copy_vals){
        void** arr = nStrides.get();
	...
	return make_intrusive<BucketCPU>(buckets_, std::move(nStrides), n_stride_size, bs, false, this->dtype_);
}



BucketCPU BucketCPU::catV(const std::vector<BucketCPU>& buckets){
	DType out_dtype = buckets[0].dtype_;
	for(const auto& ref : buckets){
		utils::throw_exception(ref.dtype_ == out_dtype, "Expected all in cat to be of the same dtype but got $ and $", out_dtype, ref.dtype_);
	}
	...
	if(convert_strides){
		...
		std::vector<BucketCPU> n_bucks(buckets.size(), BucketCPU::makeNullBucketCPU(out_dtype));
		for(uint32_t i = 0; i < buckets.size(); ++i){
			n_bucks[i] = buckets[i].bucket_all_indices_cpu();
		}
		return BucketCPU::catV(n_bucks);
	}
	...
	for(const auto& ref : buckets){
		BucketCPU::processCatData(ref, n_data, nStrides, stride_index);
	}
	return BucketCPU(to_device_holder(n_data), std::move(nStrides), nstride_s, n_data.size(), blocked, out_dtype);
}
...
BucketCPU BucketCPU::catV(const std::vector<std::reference_wrapper<const BucketCPU> >& buckets){
	DType out_dtype = buckets[0].get().dtype_;
	for(const auto& ref : buckets){
		utils::throw_exception(ref.get().dtype_ == out_dtype, "Expected all in cat to be of the same dtype but got $ and $", out_dtype, ref.get().dtype_);
	}
	...
	if(convert_strides){
		...
		std::vector<BucketCPU> n_bucks(buckets.size(), BucketCPU::makeNullBucketCPU(out_dtype));
		for(uint32_t i = 0; i < buckets.size(); ++i){
			n_bucks[i] = buckets[i].get().bucket_all_indices_cpu();
		}
		return BucketCPU::catV(n_bucks);
	}
	std::vector<std::reference_wrapper<const intrusive_ptr<Device> > > n_data;
	...
	uint64_t stride_index = 0;
	for(const auto& ref : buckets){
		BucketCPU::processCatData(ref.get(), n_data, nStrides, stride_index);
	}
...
/* BucketCPU BucketCPU::FromShared(intrusive_ptr<void[]> ptr, uint64_t s, DType dt){ */
/* 	BucketCPU output(s, dt, dCPU); */
