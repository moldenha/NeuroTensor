...
TensorGrad TensorGrad_Functional_Class::at(const TensorGrad& input, Tensor::size_value_t index){
    if(!input.track_grad()) return TensorGrad(::nt::functional::at(input.detach(), index), false);
    TensorGrad result = input.create_view_node(::nt::functional::at(input.detach(), index));
    result.create_view_backward_function(input, [&index](Tensor& grad){
...
TensorGrad TensorGrad_Functional_Class::at_tensor_split(const TensorGrad & input, const Tensor & index, Tensor::size_value_t splitting){
    if(!input.track_grad()) return TensorGrad(::nt::functional::at_tensor_split(input.detach(), index, splitting), false);
    TensorGrad result = input.create_view_node(::nt::functional::at_tensor_split(input.detach(), index, splitting));
    result.create_view_backward_function(input, [&index, &splitting](Tensor& grad){
...
TensorGrad& TensorGrad_Functional_Class::at_tensor_split(const TensorGrad & input, const Tensor & index, Tensor::size_value_t splitting,
    ...
    Tensor idx = index.conditional_mutate_clone();

    result.create_write_node(
        [index = std::move(idx), splitting = splitting]
        (const Tensor& grad, std::vector<intrusive_ptr<TensorGrad>> &parents){
            Tensor grad_ = grad.clone();
            ::nt::functional::at_tensor_split(grad_, index, splitting).fill_(0);
...
TensorGrad TensorGrad_Functional_Class::index_except(const TensorGrad & input, int64_t dim, Tensor::size_value_t excluding_index){
    if(!input.track_grad()) return TensorGrad(::nt::functional::index_except(input.detach(), dim, excluding_index), false);
    TensorGrad result = input.create_view_node(::nt::functional::index_except(input.detach(), dim, excluding_index));
    result.create_view_backward_function(input, [&dim, &excluding_index](Tensor& grad){
...
TensorGrad TensorGrad_Functional_Class::index_select(const TensorGrad & input, int64_t dim, const Tensor& index){
    if(!input.track_grad()) return TensorGrad(::nt::functional::index_select(input.detach(), dim, index), false);
    TensorGrad result = input.create_view_node(::nt::functional::index_select(input.detach(), dim, index));
    result.create_view_backward_function(input, [&dim, &index](Tensor& grad){
...
TensorGrad TensorGrad_Functional_Class::select(const TensorGrad& input, Tensor::size_value_t dim, Tensor::size_value_t index){
    if(!input.track_grad()) return TensorGrad(::nt::functional::select(input.detach(), dim, index), false);
    TensorGrad result = input.create_view_node(::nt::functional::select(input.detach(), dim, index));
    result.create_view_backward_function(input, [&dim, &index](Tensor& grad){
