...
template<typename T, typename T2>
inline void run_inner_unfoldnd_loop(
    size_t i,
    int64_t col_add,
    int64_t img_add,
        ...
        [&](threading::blocked_range<1> block) {
            for (int64_t batch = block.begin[0]; batch < block.end[0]; ++batch) {
                ...
                for (int64_t c = 0; c < channels_col; ++c) {
                    ...
                    std::vector<int64_t> indexes = pad_start;
                    run_inner_unfoldnd_loop(0, col_add, im_add, indexes,
...
void unfoldnd_(ArrayVoid &im, ArrayVoid &col, const int64_t& channels,
    ...
    im.execute_function<WRAP_DTYPES<NumberTypesL, DTypeEnum<DType::Bool> > >(
       im2col_cpu_nd, col, channels, size, kernels, strides, paddings, dilations,
                    batch_size);
}


#undef NT_INNER_UNFOLDND_TEXT__ 
