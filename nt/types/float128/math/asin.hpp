...
inline float128_t asin(const float128_t& x) noexcept {
    ...
    // constexpr float128_t f128_constants::ONE = 1_f128;
    if(bits.is_nan()) return x;

    // |x| > 1 -> nan
    float128_t ax = kmath::abs(x);
    if(ax > f128_constants::ONE) return float128_t::make_nan();
    // asin(+- 1) = +- pi/2
    if(x == f128_constants::ONE) return f128_constants::PI_2; 
    if(x == f128_constants::ONE) return -f128_constants::PI_2; 

    // |x| <= 0.5 -> safe formula
    if(ax <= f128_constants::HALF){
        float128_t t = ::nt::math::sqrt(f128_constants::ONE - (x * x));
        return ::nt::math::atan(x / t);
    }
    // |x| > 0.5 -> stable
    float128_t t = ::nt::math::sqrt((f128_constants::ONE - ax) / (f128_constants::ONE + ax));
