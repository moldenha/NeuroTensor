...
void find_is_parents_cleared(const std::vector<intrusive_ptr<TensorGrad> >& parents, bool& out){
    for(auto& parent : parents){
        if(parent->backwardFunc->is_valid() && !parent->backwardFunc->is_cleared()){
            out = false;
            return;
        }
...
void TensorGrad::backward_child(const Tensor &grad,
    ...
    // a lot of times, the only way the child is stored is within the vector it
    // is about to be removed from for this reason, the parents and the children
    // need to be coppied over and then the backward pass can proceed backward
    ...
    vec->remove(index); // removes this child, preventing a continuous loop of
                        // doing the backward pass of this child

    // this is basically the child pass of the backward_parents is done, and
    // then delete itself from any type of trace and then afterwards proceede to
...
TensorGrad::TensorGrad(Tensor t, intrusive_ptr<tensor_holder> g,
...
TensorGrad &TensorGrad::operator=(Scalar s) {
    ...
    tensor = s;
    this->track_self_mod([](const Tensor &grad,
...
TensorGrad TensorGrad::operator+(const TensorGrad &other) const {
    ...
    result.create_backward_function(
        [](const Tensor &grad,
           std::vector<intrusive_ptr<TensorGrad>> &parents) {
            // std::cout << "runing backward function for operator+"<<std::endl;
            // std::cout << "grad is "<<grad.shape()<<std::endl;
            parents[0]->grad->tensor += grad;
            // std::cout << "did parents[0]"<<std::endl;
            parents[1]->grad->tensor += grad;
            // std::cout << "did parents[1]"<<std::endl;
        });
...
void TensorGrad::redefine_tracking(
    ...
    //this will ensure there is no segmentation faults or anything if a gradient has not been initialized
}
    ...
    auto new_backward_func =
        [](const Tensor &grad, std::vector<intrusive_ptr<TensorGrad>> &parents,
           ...
           intrusive_ptr<tensor_holder> a, intrusive_ptr<tensor_holder> b) {
            ...
            parents[parent_index + 1]->grad->tensor -=
                (a->tensor * grad) / (b->tensor.pow(2));
        };
    this->track_self_mod(std::bind(new_backward_func, std::placeholders::_1,
                                   std::placeholders::_2, std::placeholders::_3,
                                   std::placeholders::_4, this_clone,
                                   other_clone),
                         other);
    return *this;
}
...
TensorGrad &TensorGrad::operator/=(const Tensor &other) {
    ...
    this->tensor /=
        other_clone->tensor; // faster because I already know it is contiguous
    auto new_backward_func = [](const Tensor &grad,
                                std::vector<intrusive_ptr<TensorGrad>> &parents,
                                const size_t parent_index, bool first,
                                intrusive_ptr<tensor_holder> b) {
        if (!first) {
    ...
    this->track_self_mod(std::bind(new_backward_func, std::placeholders::_1,
                                   std::placeholders::_2, std::placeholders::_3,
                                   std::placeholders::_4, other_clone));
    return *this;
}
...
TensorGrad TensorGrad::operator[](size_value_t i) const {
    ...
    if (tensor.dtype == DType::TensorObj && dims() == 1) {
        ...
        if(grad == nullptr){
            grad = make_intrusive<tensor_holder>(functional::zeros_like(this->tensor));
        }
        intrusive_ptr<TensorGrad> this_ptr = make_intrusive<TensorGrad>(*this);
        result->parents.clear();
        result->parents.reserve(1);
        result->parents.push_back(std::move(this_ptr));
        result->grad = make_intrusive<tensor_holder>(grad->tensor[i].item<Tensor>());

        // if(grad == nullptr){
        //     grad = make_intrusive<tensor_holder>(functional::zeros_like(tensor));
        // }
        // result.parents.clear();
        // result.parents.push_back(make_intrusive<TensorGrad>(*this));
        // result.grad = make_intrusive<tensor_holder>(this->grad->tensor[i].item<Tensor>());
        // this->children->push_back(make_intrusive<TensorGrad>(*this));
        // result.track_grad(*this,
        //                   [i](Tensor &grad) { return grad[i].item<Tensor>(); });
        return *result;
    }
...
TensorGrad TensorGrad::transpose(size_value_t a, size_value_t b) const {
    ...
    result.track_grad(*this, [a, b](Tensor &grad) { return grad.transpose(a, b); });
    return std::move(result);
}
