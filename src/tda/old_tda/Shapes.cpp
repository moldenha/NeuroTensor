...
/* void visualize(const Points& points, uint32_t height, uint32_t width, uint32_t channels, int64_t point_radius, int64_t ball_radius, bool only_biggest_shape, uint32_t shapes_above){ */
/* 	utils::throw_exception(points.dims() == 3, "Expected to visualize points of 3d but got $d", points.dims()); */
/* 	GraphDrawer drawer; */
...
/* 		drawer.drawPoint(pt[1], pt[2], pt[0]); */
/* 	} */
/* 	std::cout << "drew points"<<std::endl; */
/* 	std::vector<BasisOverlapping> balls = shapes.getBalls().getBalls(); */
...
/* 				/1* const Basis<3>& ball = bs[pt]; *1/ */
/* 				drawer.drawSphere(pt[1], pt[2],pt[0], draw_radius, color); */
/* 			} */
/* 		} */
/* 	} */
/* 	else{ */
