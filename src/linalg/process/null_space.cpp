...
Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
    svd_null_space_eigen(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& mat){
    ...
    if constexpr (detail::is_eigen_complex<T>){
        ...
        Eigen::Matrix<detail::from_complex_eigen_t<T>, Eigen::Dynamic, 1> singularValues = svd.singularValues().real(); // Singular values are always real
        // Tolerance for zero singular values
        detail::from_complex_eigen_t<T> tol = 1e-10;
        // Determine the null space basis (right singular vectors corresponding to zero singular values)
        ...
        Eigen::JacobiSVD<MatType> svd(mat, Eigen::ComputeFullV);
        Eigen::Matrix<T, Eigen::Dynamic, 1> singularValues = svd.singularValues(); // Singular values are always real
        // Tolerance for zero singular values
        T tol = 1e-10;
        // Determine the null space basis (right singular vectors corresponding to zero singular values)
        int rank = (singularValues.array() > tol).count();
...
template<typename T>
Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
    lu_null_space_eigen(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& mat){
    using MatType = typename Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
...
#define _NT_NULL_SPACE_EIGEN_DECLARE_(type) template Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> \
        svd_null_space_eigen<type>(Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>&); \
        template Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> \
        lu_null_space_eigen<type>(Eigen::Matrix<type, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>&); \
