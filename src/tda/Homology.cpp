...
std::map<double, int64_t> PersistentHomology::GetH0BettiNumbers() {
    ...
    double max_val =
        *std::max_element(reinterpret_cast<double *>(distances.data_ptr()),
                          reinterpret_cast<double *>(distances.data_ptr_end()));
    distances.fill_diagonal_(max_val + 1);
    Tensor split = distances.split_axis(-2);
    Tensor *begin = reinterpret_cast<Tensor *>(split.data_ptr());
    Tensor *end = reinterpret_cast<Tensor *>(split.data_ptr_end());
    std::vector<double> mins;
    mins.reserve(split.numel());
    for (; begin != end; ++begin) {
        mins.push_back(*std::min_element(
            reinterpret_cast<double *>(begin->data_ptr()),
            reinterpret_cast<double *>(begin->data_ptr_end())));
    }
    std::sort(mins.begin(), mins.end());
    int64_t nums = mins.size();
    std::map<double, int64_t> out;
    out[0.0] = nums;

    for (int64_t i = 0; i < mins.size(); ++i) {
        double cur = mins[i];
        if (i == mins.size() - 1) {
            out[cur] = 0;
            break;
        ...
        int64_t subtract = 1;
        while (i < mins.size() && mins[i] == cur) {
            ++subtract;
            ++i;
        }
        nums = std::max(static_cast<int64_t>(0), nums - subtract);
        out[cur] = nums;
        if (nums == 0) {
            break;
        }
...
PersistentHomology::GetH0Generators() {
    ...
    _mins.reserve(split.numel());
    for (; begin != end; ++begin) {
        _mins.push_back(*std::min_element(
            reinterpret_cast<double *>(begin->data_ptr()),
            reinterpret_cast<double *>(begin->data_ptr_end())));
    }
    std::vector<int64_t> indices(_mins.size());
    std::iota(indices.begin(), indices.end(), 0);
    std::sort(indices.begin(), indices.end(), [&_mins](int64_t i1, int64_t i2) {
        return _mins[i1] < _mins[i2];
    });
    std::vector<double> mins(_mins.size());
    for (size_t i = 0; i < mins.size(); ++i) {
        mins[i] = _mins[indices[i]];
    }
    int64_t nums = mins.size();
    const int64_t generator_size = mins.size();
    std::map<double, int64_t> out;
    std::map<double, Tensor> generators;
    out[0.0] = nums;
    generators[0.0] = functional::ones({generator_size}, DType::int8);

    for (int64_t i = 0; i < mins.size(); ++i) {
        double cur = mins[i];
        if (i == mins.size() - 1) {
            out[cur] = 0;
            generators[cur] = functional::zeros({generator_size}, DType::int8);
            break;
        }
        int64_t subtract = 1;
        while (i < mins.size() && mins[i] == cur) {
        ...
        Tensor gen = functional::ones({generator_size}, DType::int8);
        int8_t *g_begin = reinterpret_cast<int8_t *>(gen.data_ptr());
        for (int64_t j = 0; j < i; ++j) {
            g_begin[indices[j]] = 0;
        }
        generators[cur] = std::move(gen);
    }
...
PersistentHomology::getHomologyGroup(int64_t k) {
    if (k == 0) {
        ...
        while (begin != end) {
            ...
            Tensor coords = functional::where(prev->second.flatten(0, -1) != 0)
                                .item<Tensor>();
            out.emplace_back(std::move(coords), 0.0, prev->first);
        }
        auto last = r_map.rbegin();
        // Tensor coords = functional::arange(r_map.begin()->second.numel(),
        // DType::int64);
        out.emplace_back(Tensor::Null(), 0.0, last->first);
        return std::move(out);
    }
    std::vector<std::tuple<Tensor, double, double>> out;
