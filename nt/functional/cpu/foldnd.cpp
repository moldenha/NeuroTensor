        ...
        [&](threading::blocked_range<1> block) {
            for (int64_t batch = block.begin[0]; batch < block.end[0]; ++batch) {
                ...
                for (int64_t c = 0; c < channels_col; ++c) {
                    ...
                    run_inner_foldnd_loop(0, col_add, im_add, indexes,
...
void foldnd_(ArrayVoid &im, ArrayVoid &col, const int64_t& channels,
    ...
    im.execute_function<WRAP_DTYPES<NumberTypesL> >(
       col2im_cpu_nd, col, channels, size, kernels, strides, paddings, dilations,
                    batch_size);
}
